...
class SafeApiKit {
  constructor({ chainId, txServiceUrl }) {
    ...
    if (txServiceUrl) {
      ...
      const url = config_1.TRANSACTION_SERVICE_URLS[chainId]
      if (!url) {
        throw new TypeError(
  ...
  async getTransaction(safeTxHash) {
    ...
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        ...
        _SafeApiKit_txServiceBaseUrl,
        'f'
      )}/v1/multisig-transactions/${safeTxHash}/`,
      method: httpRequests_1.HttpMethod.Get
    })
  }
  /**
  ...
  async getSafeDelegates({ safeAddress, delegateAddress, delegatorAddress, label, limit, offset }) {
    const url = new URL(
      `${__classPrivateFieldGet(this, _SafeApiKit_txServiceBaseUrl, 'f')}/v1/delegates`
    )
    if (safeAddress) {
  ...
  async proposeTransaction({
    ...
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        ...
        _SafeApiKit_txServiceBaseUrl,
        'f'
      )}/v1/safes/${safe}/multisig-transactions/`,
      method: httpRequests_1.HttpMethod.Post,
      body: {
        ...safeTransactionData,
  ...
  async getIncomingTransactions(safeAddress) {
    ...
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        ...
        _SafeApiKit_txServiceBaseUrl,
        'f'
      )}/v1/safes/${address}/incoming-transfers?executed=true`,
      method: httpRequests_1.HttpMethod.Get
    })
  }
  /**
