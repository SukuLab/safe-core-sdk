...
async function predictSafeAddress({
  ...
  const {
    ...
    saltNonce = exports.PREDETERMINED_SALT_NONCE
  } = safeDeploymentConfig
  const safeProxyFactoryContract = await memoizedGetProxyFactoryContract({
    ethAdapter,
    safeVersion,
  ...
  if ([ZKSYNC_MAINNET, ZKSYNC_TESTNET].includes(chainId)) {
    ...
    return zksync_web3_1.utils.create2Address(
