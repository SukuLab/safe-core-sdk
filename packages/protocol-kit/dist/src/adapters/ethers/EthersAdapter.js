...
class EthersAdapter {
  ...
  async getBalance(address, blockTag) {
    return bignumber_1.BigNumber.from(
      await __classPrivateFieldGet(this, _EthersAdapter_provider, 'f').getBalance(address, blockTag)
    )
  }
  ...
  async getStorageAt(address, position) {
    const content = await __classPrivateFieldGet(this, _EthersAdapter_provider, 'f').getStorageAt(
      address,
      position
    )
  ...
  signMessage(message) {
    ...
    const messageArray = __classPrivateFieldGet(this, _EthersAdapter_ethers, 'f').utils.arrayify(
      message
    )
  ...
  encodeParameters(types, values) {
    return new (__classPrivateFieldGet(this, _EthersAdapter_ethers, 'f').utils.AbiCoder)().encode(
      types,
      values
    )
  }
  decodeParameters(types, values) {
    return new (__classPrivateFieldGet(this, _EthersAdapter_ethers, 'f').utils.AbiCoder)().decode(
      types,
      values
    )
  }
