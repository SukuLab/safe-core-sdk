...
class Safe {
  constructor() {
    ...
    this.getSafeMessageHash = async (messageHash) => {
      ...
      const safeMessageHash = await __classPrivateFieldGet(this, _Safe_ethAdapter, 'f').call({
        from: safeAddress,
        to: safeAddress,
        data: data || '0x'
      })
    ...
    this.isValidSignature = async (messageHash, signature = '0x') => {
      ...
      const data = fallbackHandler.encode('isValidSignature(bytes32,bytes)', [
        messageHash,
        signature && Array.isArray(signature) ? (0, utils_3.buildSignature)(signature) : signature
      ])
      try {
        const isValidSignatureResponse = await __classPrivateFieldGet(
          this,
          _Safe_ethAdapter,
          'f'
        ).call({
        ...
        return (
          isValidSignatureResponse.slice(0, 10).toLowerCase() ===
   ...
   */
  async getHash(txOrMessage) {
    if (!__classPrivateFieldGet(this, _Safe_contractManager, 'f').safeContract) {
      throw new Error('Safe is not deployed')
    }
    if (typeof txOrMessage === 'string') {
      return (0, web3_utils_1.soliditySha3)((0, web3_utils_1.utf8ToHex)(txOrMessage)) || ''
    }
   ...
   */
  async signHash(hash, isSmartContract = false) {
    const signature = await (0, utils_3.generateSignature)(
      __classPrivateFieldGet(this, _Safe_ethAdapter, 'f'),
      hash
    )
    ...
    if (isSmartContract) {
      const safeAddress = await this.getAddress()
      return new utils_2.EthSafeSignature(safeAddress, signature.data, isSmartContract)
    }
    return signature
  }
  /**
   ...
   * @returns The Safe signature
   */
  async signTypedData(eip712Data, methodVersion, isSmartContract = false) {
    let data
    ...
    const signature = await (0, utils_3.generateEIP712Signature)(
      __classPrivateFieldGet(this, _Safe_ethAdapter, 'f'),
      safeEIP712Args,
      methodVersion
    )
  ...
  async createSafeDeploymentTransaction(customSaltNonce, transactionOptions) {
    ...
    const saltNonce =
      customSaltNonce || safeDeploymentConfig?.saltNonce || utils_1.PREDETERMINED_SALT_NONCE
